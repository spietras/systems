...
buildPhase() {
	...
	find ./ -type f -path '*/Library/Fonts/*Mono*.ttf' -exec nerd-font-patcher --complete -out patched {} \;
	find ./ -type f -path '*/Library/Fonts/*Mono*.otf' -exec nerd-font-patcher --complete -out patched {} \;
}

installPhase() {
	# copy the font files to the proper directories

	mkdir --parents "${out}/share/fonts/truetype/"
